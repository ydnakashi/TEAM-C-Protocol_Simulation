...
class NetworkModel:
    ...
    def build_from_coordinates(self, coords: list[tuple[float, float]], link_range: float = 2.0) -> None:
        ...
        for i in range(n):
            ...
            # Add node to graph
            self._graph.add_node(i+1, 
                                 label=f"Node{i+1}", 
                                 node=(Node(id=node_id, powerPercent=randomBattery,coords=[coords[i][0], coords[i][1]], Rc=link_range)))
    ...
    def get_stats(self) -> NetworkStats:
        ...
        G = self._graph
        n_exists = G.number_of_nodes() > 0 
        return NetworkStats(
            num_nodes=G.number_of_nodes(),
            num_edges=G.number_of_edges(), 
            is_connected=nx.is_connected(G) if n_exists else False,
    ...
    def get_source_nodes(self) -> list[int]:
        ...
        for nd in self._graph.nodes():
            if nd == self._base_station:
                continue
            if nx.has_path(self._graph, nd, self._base_station):
                srcs.append(nd)
        return sorted(srcs)
    ...
    def init_actions(self):
        """Initialize actions to start routing"""
    ...
    def create_TDMA_schedule(self, ni):
        ...
        for id, child in node.chdList.items():
            self._graph.nodes[id]["node"].tdmaSlot = slot  # Assign slot
            child.tdma_slot = slot
            slot += 1
            total += 1
        
        node.resetWaiting()  # Reset to wait for all children again
    ...
    def calculate_worthiness_score(self, L, N, c=1):
        """Worthineess score calculations"""
        if(N == 0): return 0
        if(L > N): return 1
        t = L/N
    ...
    def startWorthinessCalc(self):
        """Calculate worthiness periodically for all nodes"""
        for ni in self._graph.nodes: 
            ...
            w_weight = 0.5
            p_weight = 0.5
            if self._protocol == Protocol.MI2RSDiC:  # Do not use worthiness for MI2RDSiC
                w_weight = 0
                p_weight = 1
    ...
    def target_destroy(self, random_destroy, id=0):
        """Mainly used for testing. Target specific nodes to destroy."""
    ...
    def init_TDMA(self):
        """Create initial TDMA schedules for all nodes before the first round of routing"""
    ...
    def redo_edges(self):
        """Visually redo edges whenever a change occurs"""
    ...
    def move_packets(self):
        """Deal with packets in transit and what happens when a packet is delivered"""
        ...
        for pkt in self._packets:
            ...
            while pkt.progress >= 1.0 and pkt.status == PacketStatus.IN_TRANSIT:
                ...
                if pkt.hop_index >= len(pkt.path) - 1:
                    ...
                    if(pkt.content["type"] == "DATA_MSG"):
                    ...
                    elif(pkt.content["type"] == "DATA_ACK"):  # node receives ACK packet
                        if node.parent is None:
                            pkt.status = PacketStatus.DROPPED
                            continue

                        # Don't use orphan election in MI2RSDiC
                        ...
                        node.parent.L += 1  # Received packet from parent, increment worthiiness
                        ...
                        # node waits a certain amount of time before doing elect_head_orphan to wait for any other orphans
                        if(node.orphan_timer == -1): node.orphan_timer = self._loss_interval
    ...
    def send_data_packet(self, ni):
        ...
        if (ni not in self._destroyed):
            ...
            # Reset for next routing or election phase
            ...
            node.resetWaiting()
            node.timer = self._loss_interval * len(node.chdList)  # Reset timer to wait for children

    def send_data_ack(self, pkt):
        """Send DATA_ACK back to child after receiving DATA_MSG"""
        node = self._graph.nodes[pkt.destination]["node"]
    ...
    def tick(self) -> SimulationSnapshot:
            ...
            if(self._phase == Phase.INIT_ROLES):
                ...
                for ni in self._graph.nodes():
                    ...
                    if(self._protocol == Protocol.TEAM_C and not node.await_parent and node.action != Action.AWAIT_REQS):
                        ...
                        if orphan_msg != None:   # Node want to trigger an orphan election
                            node.action = Action.ORPHAN_ELECTION
                    ...
                    # Determine to re-elect head or not
                        ...
                        if msg == None:  # Node does not want to re-elect
                            self.send_ready_msg(ni)
                            node.action = Action.IDLE
                            continue

                        self.send_election_msg(ni, msg)  # Node wants to re-elect
                    elif (node.action == Action.ORPHAN_ELECTION):

                        if orphan_msg != None:  # Node is an orphan. Wants to send REQUEST_PARENT to a new parent
                            self.spawn_packet(orphan_msg, ni, new_head_orphan.id)
                        else:
                            node.action = Action.ELECTION   # Proceed to normal election if still have a parent
                    
                    elif (node.action == Action.AWAIT_REQS):   # Node is chosen as a new head from an orphaned node

                        if(node.orphan_timer > 0): node.orphan_timer -= 1   # Wait a certain amount of time before electing
        ...
        # every so often have a chance to destroy a node
        # if a node is destroyeed, it gets caught in the NEXT NEXT worthiness score (as it would have had some packets being sent before it died)
        if self._tick % 150 == 0:
    ...
    def twaitCalculation(self, graphX=10, graphY=10, Rc=2, alpha=0.5):
        """Calculate the time (twait) for a node to wake up to choose an initial role"""
    ...
    def update_new_head(self, ni, msg):
        """Nodes update themselves when receiving UPDATE_HEAD message"""
        node = self._graph.nodes[ni]["node"]
        ...
        if (node.id == msg["newHead"]):
            ...
            # Add new children to its chdList
            node.chdList.update(msg["chdList"])
    ...
    def update_new_head_orphan(self, ni, msg):
        """Nodes update themselves when received an UPDATE_HEAD_ORPHAN message"""
        node = self._graph.nodes[ni]["node"]
        # If this is the new head, update itself as parent
        if (node.id == msg["newHead"]):
            # update state to new state
    ...
    def observe_parent_potential(self, ni, Oth):
        ...
        # check if parent's worthiness score is still high
    ...
    def find_closest_CH(self, node: Node):
        """Find the closest CH to a node for orphan election"""
        closest_CH = None
        nodeList = node.neighbourList
        if(node.state == NodeType.CLUSTER_HEAD):  # if a CH, use broadcastList
            nodeList = node.broadcastList

        closest_CH = self.search_lists(node, nodeList)
        
        if(closest_CH == None and node.state == NodeType.CLUSTER_HEAD):   # Expand search distance if no CH found
            closest_CH = self.search_lists(node, node.relayList)
        
        # For non-CH nodes. check neighborList first, then broadcastList, then relayList
        ...
        if(closest_CH == None):   # No CH found, default to BS as last resort
            closest_CH = self._graph.nodes[self._base_station]["node"]
        
        return closest_CH

    def search_lists(self, node: Node, list: list):
        """Search neighborList or broadcastList of a node to find the closest CH to it, if it exists"""
        closest_CH = None
        ...
        if(closest_CH == None): return None   # No CH is found
